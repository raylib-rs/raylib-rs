...
if [ "$require_date" = "--require-date" ]; then
  if ! grep -qE '^## [0-9.]+ — [0-9]{4}-[0-9]{2}-[0-9]{2}' CHANGELOG.md; then
    ...
    echo "ERROR: CHANGELOG.md heading '## X.Y.Z' must be followed by ' — YYYY-MM-DD' (em-dash U+2014 + ISO date) before real publish" >&2
...
if [ "$crate" = "raylib" ]; then
  ...
  sys_req=$(grep -E '^raylib-sys\s*=\s*.*version\s*=\s*"[0-9.]+"' raylib/Cargo.toml \
