...
fn main() {
    ...
    let mut shader = rl.load_shader(
        ...
        None,
        Some(&format!(
            "resources/shaders/shaders/glsl{GLSL_VERSION}/cubes_panning.fs"
        )),
    );
    ...
    // Detect window close button or ESC key
    {
        ...
        {
            let mut s = d.begin_shader_mode(&mut shader); // Enable our custom shader for next shapes/textures drawings
            s.draw_texture(&texture, 0, 0, Color::WHITE); // Drawing BLANK texture, all rendering magic happens on shader
