...
## raylib 6.0 upgrade (MERGED — historical context)

The 6.0 upgrade has landed on `raylib-rs/raylib-rs:unstable`. This section is preserved as the design+decision record that shaped the current codebase. **Authoritative artifacts** (load the relevant one before touching related code — they hold the full detail this summary points at):
