...
| [#251](https://github.com/raylib-rs/raylib-rs/pull/251) | Port raylib-sys to work with #![no_std] environments | nbe1233 | unstable | adapt | Post-release queue — originally targeted for WS1 adoption during the bindgen regeneration, but WS1 didn't pull in the core/std split, so the bindings are still std-bound on the merged 6.0. Re-queued post-release; see `ws8e-checkpoint-review-feedback.md` |
...
| [#234](https://github.com/raylib-rs/raylib-rs/issues/234) | Safe bindings for raylib's OpenGL draw calls | — | adapt | **DONE (WS5b)** — `raylib/src/rlgl/` safe immediate-mode module: `rl_begin`/`rl_draw` vertex stream + `rl_set_texture`/`rl_enable_shader` bind helpers. GL-object lifecycle + full state surface left to existing RAII + raw `ffi`. |
...
| [#179](https://github.com/raylib-rs/raylib-rs/issues/179) | Implement ffi::rlPushMatrix family of functions | — | adapt | **DONE (WS5b)** — `RlMatrix` RAII guard via `rl_push_matrix` (auto-`rlPopMatrix`) + `rl_translatef`/`rl_rotatef`/`rl_scalef`/`rl_load_identity`/`rl_matrix_mode`/`rl_ortho`/`rl_mult_matrixf`/`rl_set_matrix_*`. |
